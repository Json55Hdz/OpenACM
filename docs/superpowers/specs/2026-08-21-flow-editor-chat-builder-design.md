...
- **The `key={updated_at}` remount can discard unsaved manual canvas edits** — if you've dragged nodes around but haven't clicked "Guardar flujo," and then the AI updates the flow via chat, the remount re-derives fully from the server's (AI-updated) state, silently dropping your unsaved drag. Same underlying tradeoff as the point above, from the other direction.
