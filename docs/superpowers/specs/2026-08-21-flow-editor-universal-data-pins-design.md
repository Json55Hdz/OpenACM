...
**How a never-walked Get node's value gets computed:** today, `outputs[get_node_id]` is only populated when the flow walk passes through that node. A pure Get node (no flow edges at all) is never walked, so nothing would ever populate `outputs[get_node_id]` for a data edge to read. Resolution fixes this by evaluating Get nodes on demand rather than expecting them to already be in `outputs`: whenever the new field-resolution helper (see the Backend section) finds a data edge whose source is a `get`-type node, it evaluates that Get's own lookup right there — `outputs.get(get_node.config["name"])` — instead of looking up `outputs[get_node_id]`. This is a small, explicit special case (Get is the one node type this spec removes from the walk, so it's the one type that needs on-demand evaluation instead of walk-time population), not a general lazy-evaluation system for every node type.
...
- A new resolution step runs before each node handler builds its config: for every field the handler is about to read, check for a matching data edge (`kind == "data"`, `to == this node`, `toHandle == field name`) before falling back to the literal + `substitute_templates`. This is a small, shared helper used by `_run_http_node`, `_run_conditional_node`, `_run_woocommerce_node`, and the `set`-node branch in `run()`'s main loop — not five separate implementations. Per the Get-node section above, this helper special-cases a data edge whose source is a `get`-type node: instead of reading `outputs[source_id]` (which won't exist for a never-walked Get), it evaluates `outputs.get(source_node.config["name"])` directly.
