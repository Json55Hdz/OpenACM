...
def create_app() -> FastAPI:
    ...
    class TokenAuthMiddleware(BaseHTTPMiddleware):
        async def dispatch(self, request, call_next):
            ...
            if not token or token != _dashboard_token:
                return JSONResponse(
                    status_code=401,
                    content={"error": "Unauthorized. Provide a valid token."}
                )
    ...
    async def get_config():
        ...
        if "token" in config_dict.get("channels", {}).get("discord", {}):
            config_dict["channels"]["discord"]["token"] = "***" if config_dict["channels"]["discord"]["token"] else ""
        if "token" in config_dict.get("channels", {}).get("telegram", {}):
            config_dict["channels"]["telegram"]["token"] = "***" if config_dict["channels"]["telegram"]["token"] else ""
        return config_dict
    ...
    async def ws_chat(websocket: WebSocket):
        ...
        try:
            while True:
                ...
                target_type = "web" if target_channel == "web" else target_channel.split('-')[0] if '-' in target_channel else "telegram"
...
async def create_web_server(
    ...
    from openacm.core.events import (
        ...
        EVENT_TOOL_CALLED, EVENT_TOOL_RESULT,
