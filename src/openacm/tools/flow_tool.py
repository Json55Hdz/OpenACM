...
- http: {"url": str, "method": "GET"|"POST"|"PUT"|"DELETE", "headers": dict, "body": str} — llamada HTTP. Pines de flujo: entrada "default", salida "default". Pines de dato de entrada (wire-or-literal): "url", "body". Pin de dato de salida: "response" (la respuesta completa) — o, sin cablear, escribe {{http_node_id}} (el valor completo) o {{http_node_id.campo}} (un campo específico) directamente en el campo de texto.
- conditional: {"field": str, "operator": "contains"|"equals"|"is_empty"|"is_error", "value": str} — evalúa una condición. Pin de flujo de entrada: "default". NO tiene pin de flujo de salida "default" — en su lugar tiene DOS: "true" y "false". Pines de dato de entrada: "field", "value". Pin de dato de salida: "result" (el valor evaluado).
...
- set: {"name": str} — nodo puro (SIN pines de flujo, ni entrada ni salida) que guarda, bajo un nombre, el valor conectado a su pin de dato de entrada — referenciable luego como {{name}}. Pin de dato de entrada (OBLIGATORIO cablearlo — sin cable no hay nada que guardar): "value". Pin de dato de salida: "value" (el valor guardado).
...
_NODE_TYPE_VOCABULARY = (
    ...
    "set: config={name} — nodo PURO, SIN pines de flujo (ni entrada ni salida). Guarda, bajo un nombre, el valor conectado a su pin de dato de entrada 'value' (OBLIGATORIO cablearlo — sin cable no hay nada que guardar), referenciable luego como {{name}}. Pin de dato de salida: 'value'. "
